...
def create_subgraph_(ID, mat_hic, mat_chic, idx, 
    ...
    uint_hic = (hic*255).type(torch.uint8)
    g.edges['interacts_c{}'.format(str(0))].data['value'] = uint_hic[tuple(fid[0])].clone().detach().flatten()
    g.edges['interacts_c{}'.format(str(1))].data['value'] = uint_hic[tuple(fid[1])].clone().detach().flatten()
    # for i in c_list:
