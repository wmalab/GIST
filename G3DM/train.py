...
def fit_one_step(epoch, require_grad, graphs, features, cluster_weights, em_networks, ae_networks, loss_fc, optimizer, device):
    ...
    weight_r = torch.linspace(0, np.pi, steps=ncluster, dtype=torch.float, device=device)
    weight_r = torch.sin(weight_r) + 0.3
    balance_weight = cw**(0.1) # torch.softmax(cw.view(-1,), 0) # torch.ones_like(cw) # 
    ...
    for epoch in np.arange(iterations):
        ...
        for j, data in enumerate(valid_dataset):
            ...
            if epoch%3==0 and j == 0 and writer is not None and config is not None:
                ...
                x = torch.linspace(start=0.1, end=30, steps=150, device=device) # mu.max()*1.5,
                ...
                x = torch.exp(torch.linspace(start=-2.0, end=8.0, steps=150, device=device)) # mu.max()*(1+1e-4)
