...
def fit_one_step(epoch, require_grad, graphs, features, cluster_weights, em_networks, ae_networks, loss_fc, optimizer, device):
    ...
    weight_r = torch.linspace(np.pi*0.1, np.pi*0.8, steps=ncluster, dtype=torch.float, device=device)*2
    weight_r = torch.sin(weight_r) + 0.5
    ...
    if require_grad:
        loss = l_wnl + l_nll_sample # + l_stdl # + l_wnl + l_stdl
