    ...
    for i in np.arange(iterations):
        for j, data in enumerate(dataset):
            ...
            if i%5==0 and j == 0 and writer is not None and config is not None:
                ...
                # print(x1, x0)
                plot_lines(np.exp(np.cumsum(np.abs(x0+1e-4))), writer, '2,3 hop_dist/bead', step=i)
                plot_lines(np.exp(np.cumsum(np.abs(x1+1e-4))), writer, '2,3 hop_dist/center', step=i)
