...
class decoder_gmm(torch.nn.Module):
    def __init__(self, num_clusters):
        ...
        self.means = torch.nn.Parameter( -5.0*torch.ones(1), requires_grad=True)
    ...
    def forward(self, distance):
        ...
        means = torch.nn.LeakyReLU(negative_slope=0.1)(self.means)
        interval = torch.cumsum(self.interval.clamp(min=0.01), dim=0).clamp(min=0.2)
