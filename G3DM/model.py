...
class decoder_gmm(torch.nn.Module):
    def __init__(self, num_clusters):
        ...
        drange = torch.sqrt((torch.range(start=1, end=self.num_clusters)*0.5)**3) + 0.5
    ...
    def forward(self, distance, contact=None):
        ...
        dis_ms = torch.cumsum(self.distance_means.clamp(min=0.1), dim=0).clamp(min=0.5, max=100.0)
