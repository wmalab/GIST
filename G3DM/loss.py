...
class ClusterWassersteinLoss(nn.Module):
    ...
    def forward(self, pred, target, num_cluster):
        ...
        # t = torch.nn.functional.one_hot(target, num_cluster)
...
class stdLoss(nn.Module):
    ...
    def forward(self, std, cluster, num_cluster):
        ...
        weight = torch.abs(cluster - num_cluster/2) - (num_cluster/6)
        res = torch.sqrt(torch.mean(torch.exp(std*weight.view(-1,1))))
        return res
