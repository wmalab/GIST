...
def plot_lines(x, writer, item_dir, step=None):
    ...
    legend = axs.legend(*scatter.legend_elements(), loc="lower right", title="Classes")
...
def plot_histogram2d(inputs, writer, item_dir, step=None):
...
def plot_distributions(inputs, writer, item_dir, step=None):
    ...
    cmaps = ['tab20']
    ydata = np.ones((len(m.flatten())))*(-0.01) # np.arange(len(m.flatten()))*0.1
